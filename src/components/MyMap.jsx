...
import ba from "../photo/Бухта Аяя.jpg"
import bp from "../photo/Бухта Песчаная.jpg"
import vs from "../photo/Возкал Слюдянки.jpg"
import vi from "../photo/Выдрино.jpg"
import gs from "../photo/Гора Соболиная.jpg"
import kch from "../photo/Камень Черского.jpg"
import zd from "../photo/Кругобайкальская жд.jpg"
import ku from "../photo/Култук.jpg"
import li from "../photo/Листвянка.jpg"
import mb from "../photo/Мыс Бархан.jpg"
import os from "../photo/Озера Сердце.jpg"
import oc from "../photo/Озеро Сухое.jpg"
import ofr from "../photo/Озеро Фролиха.jpg"
import ol from "../photo/Остров Ольхон.jpg"
import obu from "../photo/Озеро Ушкании.jpg"
import pch from "../photo/Пик Черского.jpg"
import pb from "../photo/Порт Байкал.jpg"
import mm from "../photo/Пролив Малое Море.jpg"
import sarma from "../photo/Деревня Сарма.jpg"
import sau from "../photo/Сарминское Ущелье.jpg"
import skr from "../photo/Скрипер.jpg"
import slu from "../photo/Возкал Слюдянки.jpg"
import to from "../photo/Тёплые озёра.jpg"
import ti from "../photo/Термальные источники Хакусы.jpg"
import xu from "../photo/Хужир.jpg"
import cherm from "../photo/Чёртов мост.jpg"
import sham from "../photo/Шаман камень.jpg"
...
useEffect(() => {
  ...
  useEffect(() => {
    const loadPoints = async () => {
      const pointsWithTypes = mappoints.map(point => ({...point, type: 'point'}));
    ...
    <div>
      <div style={{ textAlign: "center", marginBottom: "10px" }}>
        ...
        <button class="setbut" onClick={() => setWaypoints([])}>Очистить маршрут</button>
          <button className={`buildRouteBut ${isTransportDropdownOpen ? 'active' : ''}`} onClick={toggleTransportDropdown}>Построить маршрут</button>
        {/* Выпадающий список */}
        {isTransportDropdownOpen && (
          <ul className="transport-dropdown">
            <li><button onClick={() => handleTransportModeChange("foot")}>Пешком</button></li>
            <li><button onClick={() => handleTransportModeChange("bike")}>На велосипеде</button></li>
            <li><button onClick={() => handleTransportModeChange("car")}>На машине</button></li>
          </ul>
        )}
      </div>
    ...
    >
      <Popup>
        ...
        {hotel.stars && <>Звезды: {hotel.stars}<br /></>}
          ...
          <div>
        <h4 className="DD">
          <DropDownMenu options={mappoints} onSelect={handleSelect}/>
        </h4>
      </div>
    </div>
